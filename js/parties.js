...
function clear(){editingId=null;$("name").value="";$("phone").value="";$("address").value="";$("saveBtn").textContent="حفظ";}
function fill(p){editingId=p.id;$("name").value=p.name||"";$("phone").value=p.phone||"";$("address").value=p.address||"";$("saveBtn").textContent="تحديث";}
...
async function del(id){if(!confirm("تأكيد الحذف؟")) return; await removeDoc(col,id); show("تم الحذف ✅",true); load();}
