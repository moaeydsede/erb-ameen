...
import { ensureSystemAccounts, systemAccountByCode, postVoucher } from "./posting.js";
import { buildCostEngine } from "./costing.js";
...
<td><select class="input" data-k="item"></select></td><td><span class="chip" data-k="uom">—</span></td>
...
<td class="chip" data-k="avg">0.00</td>
<td class="chip" data-k="total">0.00</td>
<td><button class="btn sm danger" data-act="rm">حذف</button></td>
</tr>`;}
function fillSelects(){
  const opt=items.map(i=>`<option value="${i.id}">${(i.sku||"")+" - "+(i.name||"")}</option>`).join("");
  document.querySelectorAll('#lines select[data-k="item"]').forEach(s=>s.innerHTML=opt);
}
...
function calc(){
  const rs=[...document.querySelectorAll("#lines tr")];
  let total=0, cogs=0;
  rs.forEach(r=>{
    const itemId=r.querySelector('[data-k="item"]').value;
    const q=Number(r.querySelector('[data-k="qty"]').value||0);
    const p=Number(r.querySelector('[data-k="price"]').value||0);
    const t=q*p;
    const a=avgCost(itemId);
    r.querySelector('[data-k="avg"]').textContent=fmt(a);
    r.querySelector('[data-k="total"]').textContent=fmt(t);
    total+=t; cogs+=q*a;
  });
  ...
  $("total").textContent=fmt(total); $("net").textContent=fmt(net); $("cogs").textContent=fmt(cogs);
}
function lines(){
  return [...document.querySelectorAll("#lines tr")].map(r=>({
    itemId:r.querySelector('[data-k="item"]').value,
    qty:Number(r.querySelector('[data-k="qty"]').value||0),
    unitPrice:Number(r.querySelector('[data-k="price"]').value||0),
  })).filter(x=>x.itemId&&x.qty>0&&x.unitPrice>0);
}
async function loadData(){
  ...
  if(whSel){
    ...
    whSel.innerHTML=warehouses.filter(w=>w.active!==false).map(w=>`<option value="${w.id}">${w.name}</option>`).join("");
  }
...
async function save(){
  const customerId=$("customer").value; const date=$("date").value||todayISO(); const day=dayNameFromISO(date);
  const warehouseId = document.getElementById("warehouse")?.value || ""; const discountValue=Number($("discount").value||0); const discountType=($("discountType")?.value||"value");
  const ls=lines(); if(!customerId) return show("اختر عميل"); if(!ls.length) return show("أضف بنود مع السعر");
  const enriched=ls.map(l=>{const it=items.find(x=>x.id===l.itemId); const a=avgCost(l.itemId); return {...l,name:it?it.name:"",sku:it?it.sku:"",avgCost:a,lineTotal:l.qty*l.unitPrice,lineCogs:l.qty*a};});
  ...
  // مرتجع مبيعات: يعيد للمخزون (IN) + المبالغ تعتبر سالبة في التقارير
  const id=await upsert("invoices",null,{type:"sales-return",date,day,customerId,discount,total,net,cogs,lines:enriched});
  // قيود محاسبية تلقائية + COGS (AVG)
  try{
    const accountsAll = await ensureSystemAccounts();
    const INV = systemAccountByCode(accountsAll,"1100"); // المخزون
    const COGS = systemAccountByCode(accountsAll,"5100"); // تكلفة
    const SALES = systemAccountByCode(accountsAll,"4100"); // المبيعات
    const AR = systemAccountByCode(accountsAll,"1200"); // العملاء
    const AP = systemAccountByCode(accountsAll,"2100"); // الموردون
    // حساب تكلفة AVG
    const costEngine = await buildCostEngine();
    let costTotal = 0;
    for(const l of enriched){
      const avg = costEngine.getUnitCost(warehouseId, l.itemId, id) || 0;
      costTotal += (avg * Number(l.qty||0));
    }
    // مرتجع مبيعات (عكس)
    if(AR && SALES){
      await postVoucher({date,day,desc:"مرتجع مبيعات",refType:"invoice",refId:id,lines:[
        {accountId:SALES.id,debit:Number(net||0),credit:0,note:"عكس مبيعات"},
        {accountId:AR.id,debit:0,credit:Number(net||0),note:"عكس ذمم"}
      ]});
    }
    if(COGS && INV && costTotal>0){
      await postVoucher({date,day,desc:"عكس تكلفة مرتجع مبيعات",refType:"invoice",refId:id,lines:[
        {accountId:INV.id,debit:Number(costTotal),credit:0,note:"عودة مخزون"},
        {accountId:COGS.id,debit:0,credit:Number(costTotal),note:"عكس COGS"}
      ]});
    }
  }catch(e){ console.warn(e); }

  // حركة مخزون حسب المستودع
  try{
    if(warehouseId){
      for(const l of enriched){
        await upsert("moves",null,{itemId:l.itemId,dir:"in",qty:Number(l.qty||0),date,day,note:"فاتورة",warehouseId,source:"invoice",refId:""});
      }
    }
  }catch(e){}
  ...
  // حركة مخزون (IN)
  for(const l of enriched){
    await upsert("inventoryMoves",null,{date,day,type:"in",reason:"sales-return",itemId:l.itemId,qty:l.qty,unitCost:l.avgCost,ref:""});
  }
  show("تم حفظ مرتجع المبيعات ✅",true);
}
