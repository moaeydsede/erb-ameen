...
(async()=>{
  ...
  inv.forEach(i=>{
    if(i.type==="sales" && m.has(i.customerId)) m.get(i.customerId).net+=Number(i.net||0);
    if(i.type==="sales-return" || i.type==="sales_return"){ if(m.has(i.customerId)) m.get(i.customerId).net-=Number(i.net||0); }
  });
  vou.forEach(v=>{
    if(v.type==="receipt"){
      (v.lines||[]).forEach(l=>{ if(m.has(l.accountId)) m.get(l.accountId).paid+=Number(l.credit||0); });
    }
  });
