...
async function save(){
  ...
  try{
    ...
    // قيد المبيعات
    if(AR && SALES){
      await postVoucher({date,day,desc:"فاتورة مبيعات",refType:"invoice",refId:id,lines:[
        {accountId:AR.id,debit:Number(net||0),credit:0,note:"ذمم"},
        {accountId:SALES.id,debit:0,credit:Number(net||0),note:"مبيعات"}
      ]});
    }
    // قيد التكلفة
    if(COGS && INV && costTotal>0){
      await postVoucher({date,day,desc:"تكلفة فاتورة مبيعات",refType:"invoice",refId:id,lines:[
        {accountId:COGS.id,debit:Number(costTotal),credit:0,note:"COGS"},
        {accountId:INV.id,debit:0,credit:Number(costTotal),note:"مخزون"}
      ]});
    }
