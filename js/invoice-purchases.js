...
async function save(){
  ...
  const id=await upsert("invoices",null,{type:"purchases",date,day,supplierId,discountType,discountValue,discount,total,net,lines:enriched});
  ...
  try{
    ...
    // قيد المشتريات (مخزون/موردين)
    if(INV && AP){
      await postVoucher({date,day,desc:"فاتورة مشتريات",refType:"invoice",refId:id,lines:[
        {accountId:INV.id,debit:Number(net||0),credit:0,note:"مخزون"},
        {accountId:AP.id,debit:0,credit:Number(net||0),note:"ذمم دائنة"}
      ]});
    }
  ...
  show("تم حفظ فاتورة المشتريات ✅ رقم: "+id,true);
}
