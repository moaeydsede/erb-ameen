...
function build(){
  ...
  vouchers.forEach(v=>{
    ...
    if(v.type==="receipt"){
      ...
      if(v.cashAccountId===accId){
        if(Number(v.sumRec||0)>0) rows.push({date:v.date,day:v.day,desc:"سند يومية (مقبوضات)",debit:Number(v.sumRec||0),credit:0});
        if(Number(v.sumPaid||0)>0) rows.push({date:v.date,day:v.day,desc:"سند يومية (مدفوعات)",debit:0,credit:Number(v.sumPaid||0)});
      }
      (v.lines||[]).forEach(l=>{
        if(l.accountId!==accId) return;
        rows.push({date:v.date,day:v.day,desc:"سند يومية",debit:Number(l.debit||0),credit:Number(l.credit||0)});
      });
  ...
  invoices.forEach(inv=>{
    ...
    if(inv.type==="sales_return" && inv.customerId===accId) rows.push({date:inv.date,day:inv.day,desc:"مرتجع مبيعات",debit:0,credit:Number(inv.net||0)});
    ...
    if(inv.type==="purchase_return" && inv.supplierId===accId) rows.push({date:inv.date,day:inv.day,desc:"مرتجع مشتريات",debit:Number(inv.net||0),credit:0});
  });
