...
async function save(){await setDoc(doc(db,"company","main"),{name:$("name").value.trim()||"ERP PRO",language:$("language").value,baseCurrency:$("baseCurrency").value.trim()||"EGP",multiCurrency:$("multiCurrency").value==="true",pricingPolicy:$("pricingPolicy").value,updatedAt:serverTimestamp()},{merge:true});show("تم الحفظ ✅",true);}
