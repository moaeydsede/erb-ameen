...
function row(){return `<tr>
  ...
  <td><input class="input" data-k="note" placeholder="سبب"></td>
...
function fill(){
  ...
  $("wh").innerHTML = whs.filter(w=>w.active!==false).map(w=>`<option value="${w.id}">${w.name}</option>`).join("");
}
...
async function save(){
  ...
  const warehouseId=$("wh").value;
  const dir=$("dir").value;
  const ls=lines();
  if(!warehouseId) return show("اختر مستودع",false);
  if(!ls.length) return show("أضف بنود",false);
  const memoId=await upsert("stockMemos",null,{type:"adjust",dir,date,day,warehouseId,lines:ls});
  for(const l of ls){ await upsert("moves",null,{itemId:l.itemId,dir,qty:l.qty,date,day,note:l.note||"تسوية",warehouseId,source:"adjust",refId:memoId}); }
  show("تم الحفظ ✅",true);
