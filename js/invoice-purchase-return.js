...
async function save(){
  const supplierId=$("supplier").value; const date=$("date").value||todayISO(); const day=dayNameFromISO(date);
  ...
  const id=await upsert("invoices",null,{type:"purchase-return",date,day,supplierId,discount,total,net,lines:enriched});
  ...
  try{
    ...
    // مرتجع مشتريات (عكس)
    if(INV && AP){
      await postVoucher({date,day,desc:"مرتجع مشتريات",refType:"invoice",refId:id,lines:[
        {accountId:AP.id,debit:Number(net||0),credit:0,note:"عكس ذمم"},
        {accountId:INV.id,debit:0,credit:Number(net||0),note:"عكس مخزون"}
      ]});
    }
  ...
  try{
    if(warehouseId){
      for(const l of enriched){
        await upsert("moves",null,{itemId:l.itemId,dir:"out",qty:Number(l.qty||0),date,day,note:"فاتورة",warehouseId,source:"invoice",refId:""});
      }
    }
  }catch(e){}
  ...
  for(const l of enriched){
    await upsert("inventoryMoves",null,{date,day,type:"out",reason:"purchase-return",itemId:l.itemId,qty:l.qty,unitCost:l.unitCost,ref:""});
  }
